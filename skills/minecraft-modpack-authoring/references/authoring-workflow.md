...
See `packwiz-workflows.md` for exact Packwiz operations and `modpack-content-and-testing.md` for configuration, data, progression, testing, and release guidance.
...
When uncertain, block release until documentation and isolated dedicated-server/client testing establish the side; do not deploy an unknown mod to a live server with `both` as a supposedly safe fallback. A false client-only classification can produce missing registries or rejected connections; a false both-side classification can crash a dedicated server by loading client classes.
...
KubeJS 2001 script reload clears and re-registers listeners, but does not replay registry or datapack lifecycles. `/kubejs reload server_scripts` is not a substitute for `/reload` for recipes/tags/loot. Check the exact version’s implementation rather than assuming listeners are immutable.
...
packwiz migrate loader <exact-target-loader-version>
```
